"""
Prompt templates for the RLM engine.

Faithfully adapted from the paper's Appendix C (arxiv 2512.24601).
"""
...
USER_PROMPT = (
    ...
    '(which contains the context) to answer the original query: "{query}".\n\n'
...
def build_system_prompt(
    ...
    context_lengths: str = "N/A",
) -> List[Dict[str, str]]:
    """Build the system prompt with context metadata interpolated."""
...
def next_action_prompt(
    ...
    """Build the per-iteration user prompt."""
    if final_answer:
        return {
            "role": "user",
            "content": (
                "Based on all the information you have gathered, provide a final "
                "answer to the user's query. Use FINAL(answer) or FINAL_VAR(variable_name)."
            ),
        }
    ...
    if iteration == 0:
        safeguard = (
            ...
            "Your next action should be to look through it -- don't just provide a final "
            "answer yet.\n\n"
        )
        ...
        return {"role": "user", "content": prefix + USER_PROMPT.format(query=query)}
